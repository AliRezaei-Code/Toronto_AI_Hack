...
import { processEdit, Transcript } from "@/src/lib/api-client";
import {
  findWordAtTime,
  formatTimestamp,
  isTranscriptEmpty,
  ...
  getWordCount,
  getAllWords,
} from "@/src/lib/transcript-utils";
import { useEffect, useState, useRef, useCallback, KeyboardEvent } from "react";
import { motion } from "framer-motion";
import { editTranscriptText } from "@/src/lib/api-client";
...
export function TranscriptEditor({
  ...
  const handleInlineWordChange = (index: number, newWord: string) => {
    ...
    const allWords = getAllWords(transcript);
    const updatedWords = inlineEditedWords.length
      ? [...inlineEditedWords]
      : [...allWords];
    const baselineWords = baselineWordsRef.current.length
  ...
  const isNearCurrentWord = (
    ...
    return (
      Math.abs(currentWordLocation.wordIndex - wordIdx) <= 2 &&
    ...
    return (
      ...
      >
        ...
        >
          ...
          <br />
          Upload videos to generate transcript.
        </motion.p>
      </motion.div>
    );
  }
          ...
          <div className="text-lg leading-relaxed transcript-panel">
            {displayWords &&
              displayWords.map((word, index) => {
                ...
                const wordLocation = transcript
                  ? findWordAtTime(transcript, word.start)
                  : null;
                const isHighlighted =
                  wordLocation && currentWordLocation
                    ? wordLocation.clipIndex ===
                        currentWordLocation.clipIndex &&
                      wordLocation.segmentIndex ===
                        currentWordLocation.segmentIndex &&
                  ...
                  >
                    {isBeingEdited ? (
                      <input
                        ...
                        value={word.word}
                        onChange={(event) =>
                          handleInlineWordChange(index, event.target.value)
                        }
                        ...
                        style={{
                          width: `${Math.max(word.word.length * 0.6, 1)}em`,
                        }}
                      />
                    ) : (
                      word.word
          ...
          {!isSidebarEdit && (
            <>
              <div className="mt-4 mx-4 text-sm text-gray-500">
                <p>
                  Double-click any word to edit. Auto-saves after you stop
                  typing. Press Enter to save now.
                </p>
              </div>
